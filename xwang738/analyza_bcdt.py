...
def tune_SVM_2(X, y):
    ...
    plot_curve(train_sizes, train_scores_mean_lc, train_scores_std_lc, cv_scores_mean_lc, cv_scores_std_lc,
               title='Learning Curve with SVM (RBF, gamma= %.4f, c=%.1f)' % (optimal_gamma, optimal_c),
               x_label='number of samples', x_lim=(0, 500), y_lim=(0.8, 1.05))
...
def tune_decisionTree(X, y):
    ...
    plot_curve(depth_range, train_scores_mean, train_scores_std, cv_scores_mean, cv_scores_std,
               ...
               x_lim=(0, 35), y_lim=(0.80, 1.05))

    # generates learning curve
    train_sizes = np.linspace(.1, 1.0, 10)  # print train_sizes.shape
...
def tune_boost(X, y):
    ...
    # Draw heatmap of the cross validation accuracy as a function of gamma and C
    plot_heatmap(scores, n_estimators_range, rate_range, vmin=0.5, mid=0.92, xlabel='n_estimators',
    ...
    plot_curve(train_sizes, train_scores_mean_lc, train_scores_std_lc, cv_scores_mean_lc, cv_scores_std_lc,
               title='Learning Curve with Adaboosting(%d of estimators, %.3f learning rate)' % (
                   optimal_n_estimators, optimal_lr), x_label='number of samples', x_lim=(0, 500), y_lim=(0.8, 1.05))
...
def tune_NN(X, y):
    ...
    hidden_layer_sizes_range = [(5,),(10,), (20,), (100,), (200,), (300,)]
    n_hidden_units = [5, 10, 20, 100, 200, 300]
    ...
    # Draw heatmap of the cross validation accuracy as a function of gamma and C
    plot_heatmap(scores, n_hidden_units, alpha_range, vmin=0.7, mid=0.92, xlabel='n_hidden_units',
    ...
    plot_curve(train_sizes, train_scores_mean_lc, train_scores_std_lc, cv_scores_mean_lc, cv_scores_std_lc,
               title='Learning Curve with Neural Network(%d hidden units and %.5f alpha)' % (
                   optimal_hls[0], optimal_alpha), x_label='number of samples', x_lim=(0, 500), y_lim=(0.8, 1.05))
...
def plot_heatmap(scores, x_range, y_range, **kwargs):
    ...
    plt.savefig(os.path.join('exported figures/bcdt', figure_name), dpi=72)
    plt.show()
...
if __name__ == '__main__':
    dataset3 = "breast-cancer-wisconsin.csv"
    X, y = preprocessing_data(dataset3)
    #print X.head(1)
